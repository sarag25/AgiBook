...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "..", "src", "agibot_x2_pkg", "meshes", "books")
...
def load_image(filename):
    """
    Load an image from the IMAGES_DIR and return a Blender image object.
    """
...
def assign_uvs(obj):
    ...
    Spine on -X (left when looking at the cover); +X for manga.
    Top +Z and Bottom -Z.
    ...
    for face in bm.faces:
        ...
        if ax == 1:
            ...
            # Faces X small: spine (-X left when looking at cover; +X right for manga) and pages
...
def assign_materials(obj, is_manga=False):
    ...
    Slot materials:
        - 0 = cover  (+Y)
        - 1 = retro  (-Y)
        - 2 = spine  (-X left when looking at cover; +X for manga)
...
def create_book(book, location=(0, 0, 0)):
    """
    Create a book mesh in Blender with the specified textures and export it as a .glb file.
    """
    print(f"\n=== {book['name']} ===")
    sx, sy, sz = book["size"]

    img_cover = load_image(book["cover"])
    img_retro = load_image(book["retro"])
    img_spine = load_image(book["spine"])
    ...
    assign_materials(obj, is_manga=book.get("manga", False))

    out_path = os.path.join(OUTPUT_DIR, book["name"] + ".glb")
...
def main():
    ...
    print("\nCreation done! File .glb saved in:", OUTPUT_DIR)
