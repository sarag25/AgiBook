"""
extract_text_llama.py
======================
Estrae il testo visibile in un ritaglio di libro usando un modello
multimodale (meta-llama/Llama-4-Scout-17B-16E-Instruct) via Inference API
di Hugging Face, al posto di un OCR tradizionale (PaddleOCR: rimosso, vedi
Sorting.md — crashava con SIGILL su CPU senza AVX-512, oltre a leggere
peggio i crop rispetto a questo modello).
...
Il modello è gated su Hugging Face (licenza Meta): va accettata sul sito
https://huggingface.co/meta-llama/Llama-4-Scout-17B-16E-Instruct con
l'account associato a HF_TOKEN prima che le chiamate funzionino — stesso
token già usato da detect_sam3.py per facebook/sam3.

Uso:
    from extract_text_llama import extract_text
...
def extract_text(image_path: str) -> str:
    """Estrae il testo visibile in image_path con Llama-4-Scout."""
    ...
    if not hf_token:
        raise RuntimeError(
            "HF_TOKEN mancante in .env: richiesto per Llama-4-Scout "
    ...
    try:
        ...
        print(f"  Llama-4-Scout non disponibile ({exc}): nessun testo estratto da {image_path}.")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python extract_text_llama.py <image_path>")
        sys.exit(1)
